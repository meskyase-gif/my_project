...
# የገጽታ ማስተካከያ
st.set_page_config(page_title="EduFinance - School & Payten Management", layout="wide")

# የዳታቤዝ ግንኙነት መረጃ (የራስህን ፓስወርድ እዚህ ይተካ)
DB_CONFIG = {
    "host": "localhost",
    "database": "school_register",
    "user": "postgres",
    "password": "654321",  # <- የአንተን pgAdmin ፓስወርድ እዚህ ያስገቡ
    "port": 5432
}

# ከዳታቤዝ ጋር የሚያገናኝ ፈንክሽን
def get_db_connection():
    return psycopg2.connect(**DB_CONFIG)

# --- ዋና የጎን ማውጫ (Sidebar Dashboard) ---
st.sidebar.title("🌐 EduFinance")
st.sidebar.caption("የተማሪዎች ምዝገባ እና የPayten ክፍያ መቆጣጠሪያ")
...
if menu == "📊 ዳሽቦርድ (Dashboard)":
    st.title("📈 አጠቃላይ የEduFinance መረጃ መቆጣጠሪያ")
    ...
    try:
        ...
        st.markdown("---")
        st.subheader("🔄 የቅርብ ጊዜ የዳታቤዝ ሁነቶች (Recent Activities)")
        
        # የቅርብ ክፍያዎችን በሰንጠረዥ ማሳያ
        conn = get_db_connection()
        ...
        if not recent_df.empty:
            ...
            st.info("እስካሁን በዳታቤዙ ላይ የተመዘገበ አዲስ እንቅስቃሴ የለም።")
...
# ==========================================
    ...
    with tab1:
        ...
        try:
            conn = get_db_connection()
            if search_query:
                df = pd.read_sql(f"SELECT id, full_name, email, grade, parent_name, parent_phone, status FROM public.students WHERE full_name ILIKE '%{search_query}%' ORDER BY id DESC;", conn)
            else:
                df = pd.read_sql("SELECT id, full_name as \"የተማሪ ስም\", email as \"ኢሜይል\", grade as \"ክፍል\", parent_name as \"የወላጅ ስም\", parent_phone as \"ስልክ\", status as \"ሁኔታ\" FROM public.students ORDER BY id DESC;", conn)
            conn.close()
                ...
                if s_name.strip() and s_email.strip() and s_parent.strip() and s_phone.strip():
                    try:
                        ...
                        cursor.execute("""
                            ...
                            VALUES (%s, %s, %s, %s, %s, %s);
                        """, (s_name.strip(), s_email.strip(), s_grade, s_parent.strip(), s_phone.strip(), s_status))
                        conn.commit()
                        cursor.close()
                        conn.close()
                        st.success(f"🎉 ተማሪ '{s_name}' በተሳካ ሁኔታ ዳታቤዝ ውስጥ ተመዝግቧል!")
                    except Exception as e:
...
# ==========================================
    ...
    with tab3:
        ...
        try:
            ...
            student_options = {row['full_name'] + f" ({row['grade']})": row['id'] for _, row in students_df.iterrows()}
...
# ==========================================
    ...
    st.info("ይህ ክፍል ከPayten የክፍያ መተላለፊያ ጋር የተገናኘ ሲሆን፣ የተማሪዎችን ያልተከፈለ ክፍያ በካርድ፣ በባንክ ወይም በሞባይል ገንዘብ ለመቀበል ያገለግላል።")
    ...
    with tab1:
        try:
            ...
            if not pending_list.empty:
                ...
                method = st.selectbox("የክfpያ መፈጸሚያ መንገድ (Payment Method)", ["Credit/Debit Card", "Bank Transfer", "Mobile Money"])
                
                if st.button("🚀 ክፍያውን በPayten ፈጽም"):
                    row_data = pay_options[selected_pay]
                    
                    # የPayten ልዩ መለያ ቁጥር በዘፈቀደ ማመንጫ (Simulation)
                    ...
                    # 2. የAssignment ሁኔታን ወደ 'Paid' መቀየር
