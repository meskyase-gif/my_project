...
st.set_page_config(page_title="CBSS Task Man't System", page_icon="📅", layout="wide")

st.markdown("""
    <div style='background-color: #1B365D; padding: 15px; border-radius: 10px; margin-bottom: 20px;'>
        <h1 style='text-align: center; color: white; margin: 0; font-family: sans-serif;'>📅 ዕለታዊ ስራዎች መቆጣጠሪያ ሲስተም</h1>
    </div>
...
if 'cbs_list' not in st.session_state:
    CBSS = ["Fikiremariam Nigatu",	"Betelhem Yosef", "Samuel Assefa", "Abebe Wondwosen", "Hailemariam Tefera", "Alamin Esayas", "Yohans Shegaw", "Endale Mekonnen", "Tedla Bayou",	"Emuye Bawok", "Tiruwork Kassa", "Samuel Getachew",	"Habtamu Kassa", "Soliana Daniel", "Mohammed Bedru", "Tirukelem Mekuriaw",	"Meseret Melkamu",	"Yonas Asmare ", "Samuel Berihu",	"Elias Mengistu",	"Yeabsira Ayalew",	"Natnael Habte",	"Ephrem Hirko ",	"Selam Kebede",	"Bawoke Mekonene",	"Mekdes Girma",	"Asmiro Sisay"] 
    st.session_state.cbs_list = sorted(CBSS)
# 2. ዋና ዳታዎችን ማዘጋጀት
...
if not st.session_state.logged_in:
    ...
    if is_admin:
        ...
        with st.sidebar.expander("🏢 የCBSS ማስተዳደሪያ"):

            
            st.markdown("**➕ አዲስ CBSS ጨምር**")
            new_dept = st.text_input("የCBSS ስም:", key="add_dept_input")
            if st.button("CBSS መዝግብ", use_container_width=True):
                if new_dept.strip() == "":
                    st.error("ስም ባዶ መሆን አይችልም!")
                elif new_dept.strip() in st.session_state.cbs_list:
                    st.warning("ይህ CBSS ስም አስቀድሞ አለ!")
                else:
                    st.session_state.cbs_list.append(new_dept.strip())
                    # 🛠️ ማስተካከያ፦ አዲስ ሲጨመር ዝርዝሩን እንደገና በፊደል ተራ መደርደር
                    st.session_state.cbs_list = sorted(st.session_state.cbs_list)
                    st.success(f"🏢 {new_dept} ተጨምሯል!")
                    st.rerun()
            
            st.markdown("---")
            st.markdown("**🗑️ CBSS አስወግድ/ሰርዝ**")
            dept_to_delete = st.selectbox("የሚሰረዝ CBSS:", st.session_state.cbs_list, key="del_dept_select")
            if st.button("ይህንን CBSS ስም ሰርዝ", type="primary", use_container_width=True):
                if len(st.session_state.cbs_list) > 1:
                    st.session_state.cbs_list.remove(dept_to_delete)
                    st.warning(f"🗑️ {dept_to_delete} ተሰርዟል!")
                    st.rerun()
                else:
                    st.error("ቢያንስ አንድ CBSS ስም መኖር አለበት!")
    ...
    with tab1:
        ...
        with col1:
            ...
            t_dept = st.selectbox("CBSS (Core Banking System Support ):", st.session_state.cbs_list)
            t_owner = st.selectbox("CBSS (Core Banking System Support ):", st.session_state.cbs_list)
            ...
            columns_name = ["Task ID", "Task Description", "Core Banking System Support ", "Assigned To", "Priority", "Status", "Estimated Hours", "Assign Date", "End Date", "Reference ID", "Verification Status"]
            ...
            st.download_button(
                ...
                file_name="CBSS_Daily_Task_Schedule_System.xls",
    ...
    with tab2:
        ...
        df_weekly_view = df_all[["Task ID", "Task Description", "Core Banking System Support ", "Assigned To", "Assign Date", "End Date", "Reference ID", "Status"]]
    ...
    with tab3:
        ...
        st.write("---")
        st.subheader("💡 የCBSS አጠቃላይ የሥራ ሁኔታ ግራፍ")
