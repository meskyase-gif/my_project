...
st.title("🌶️ የMeski-Flavor ")

# የዳታቤዝ ግንኙነት መረጃ
DB_CONFIG = {
    "host": "localhost",
    "database": "testflavor_py_db",
    "user": "postgres",
    "password": "654321",
    "port": 5432
}
...
def save_product(name, batch, qty, prod_date, expir_date, status):
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        
        # ለQR ኮድ የሚሆን ልዩ መረጃ ማዘጋጀት
        qr_data = f"Product: {name} | Batch: {batch} | Qty: {qty}kg | Date: {prod_date} | Date: {expir_date} | Status: {status}"
        
        # ስሙ ወደ public.flavor_products ተስተካክሏል
        query = """
            INSERT INTO public.flavor_products (product_name, batch_number, quantity_kg, production_date, Expired_date, quality_status, qr_code_data)
            VALUES (%s, %s, %s, %s, %s, %s, %s);
        """
        cursor.execute(query, (name, batch, qty, prod_date, expir_date, status, qr_data))
        conn.commit()
        cursor.close()
        conn.close()
        return True
...
with tab1:
    ...
    e_date = st.date_input("ያለቀበት ቀን (Expired Date)")
    ...
    if st.button("💾 ምርቱን መዝግብ እና QR ኮድ አውጣ"):
        if p_name.strip() and b_num.strip():
            success = save_product(p_name.strip(), b_num.strip(), p_qty, p_date, e_date, p_status)
            if success:
...
with tab2:
    ...
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        # ስሙ ወደ public.flavor_products ተስተካክሏል
        df = pd.read_sql("SELECT id, product_name, batch_number, quantity_kg, production_date, Expired_date, quality_status FROM public.flavor_products ORDER BY id DESC;", conn)
        conn.close()
        ...
        st.error(f"መስኪ ሪፖርቱን ለማንበብ አልተቻለም፦ {e}")