...
st.set_page_config(page_title="Meski School System", layout="centered")

# 1. የዳታቤዝ ስም ወደ ትክክለኛው 'school_py_db' ተስተካክሏል
DB_CONFIG = {
    "host": "localhost",
    "database": "test_py_db",
    "user": "postgres",
    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS public.students (
                ...
                student_location VARCHAR(20) NOT NULL,
...
with tab1:
    ...
    s_location = st.text_input("LOCATION (Student location)")
    ...
    if register_btn:
        if s_name.strip():
            try:
                ...
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO public.students (student_name, grade_level, parent_phone, student_location)
                    VALUES (%s, %s, %s, %s)
                ''', (s_name.strip().upper(), s_grade, s_phone.strip(), s_location.strip())) #  ቅንፍ ተወግዶ ወደ .strip() ተስተካክሏል
                conn.commit()
                cursor.close()
                conn.close()
...
try:
    conn = psycopg2.connect(**DB_CONFIG)
    #  ከ FROM በፊት የነበረችው ትርፍ ኮማ ተወግዳለች
    query = '''
        SELECT id AS "መለያ ቁጥር",
               student_name AS "የተማሪ ስም", 
               grade_level AS "ክፍል",
               student_location AS "LOCATION",
